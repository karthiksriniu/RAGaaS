...
export type DeliveryChannel = "vobiz-voice" | "whatsapp" | "none";

function serviceSid(): string | null {
  return process.env.TWILIO_VERIFY_SERVICE_SID || null;
}

function vobizVoiceReady(): boolean {
  return (
    !!process.env.VOBIZ_AUTH_ID &&
    !!process.env.VOBIZ_AUTH_TOKEN &&
    !!process.env.OTP_CALLER_NUMBER &&
    !!process.env.APP_BASE_URL
  );
}

function whatsappReady(): boolean {
  return !!serviceSid() && !!process.env.TWILIO_ACCOUNT_SID && !!process.env.TWILIO_AUTH_TOKEN;
}

/** Which channel this environment can actually deliver on.
 *
 * Vobiz voice is preferred over WhatsApp when both are available. That is the
 * deliberate India-first ordering: Vobiz is already carrying the calls and the
 * numbers, so it is one fewer vendor in the critical path, and a voice call
 * reaches a feature phone. WhatsApp stays configured as the path for customers
 * outside India, where Vobiz does not reach.
 *
...
export function configuredChannel(): DeliveryChannel {
  if (vobizVoiceReady()) return "vobiz-voice";
  if (whatsappReady()) return "whatsapp";
  return "none";
}

/** Which settings each channel still needs, by NAME.
 *
 * "It is configured but reports none" is otherwise a guessing game across four
 * or five variables, and every guess costs a redeploy. Names only - never
 * values - so this carries nothing secret, but the route still only exposes it
 * on staging, because a config gap is not something to advertise in public. */
export function missingSettings(): Record<DeliveryChannel, string[]> {
  const absent = (...names: string[]) => names.filter((n) => !process.env[n]);
  return {
    "vobiz-voice": absent("VOBIZ_AUTH_ID", "VOBIZ_AUTH_TOKEN", "OTP_CALLER_NUMBER", "APP_BASE_URL"),
    whatsapp: absent("TWILIO_VERIFY_SERVICE_SID", "TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN"),
    none: [],
  };
}

/** Calls the owner and reads the code aloud.
 *
 * The code is generated by us for this channel (unlike WhatsApp, where Twilio
 * owns it), so verification stays on the existing local path with its expiry
 * and attempt cap. */
export async function startVoiceVerification(mobile: string, code: string): Promise<void> {
  const { placeOtpCall } = await import("@/lib/vobiz");
  const { encodeOtpVoiceToken } = await import("@/lib/otpVoiceToken");

  const base = (process.env.APP_BASE_URL || "").replace(/\/$/, "");
  const token = encodeOtpVoiceToken({ code, mobile });
  await placeOtpCall(mobile, `${base}/api/business/otp/voice?t=${encodeURIComponent(token)}`);
}
